...
const registerStudent = asyncHandler(async (req, res) => {
  try {
    const { email, firstname, lastname, age, nationality, guardName, guardEmail, gender, roomNum } = req.body;

    if (!email || !firstname || !lastname || !age || !nationality || !guardName || !guardEmail || !gender || !roomNum) {
      res.status(400);
...
const getAllStudents = asyncHandler(async (req, res) => {
  ...
  const updateStudentProfile = asyncHandler(async (req, res) => {
    ...
    if (student) {
      const { email, firstname, lastname, age, nationality, guardian , gender,} = student;
        
      student.email = email
      student.firstname = req.body.firstname || firstname;
      student.lastname = req.body.lastname || lastname;
      student.age = req.body.age || age;
      student.nationality = req.body.nationality || nationality;
      student.guardian.guardianName = req.body.guardName || guardian.guardianName;
      student.guardian.guardianEmail = req.body.guardEmail || guardian.guardianName;
...
module.exports = { registerStudent, getAllStudents, getStudent, updateStudentProfile, changeStudentRoom, updateCheckInStatus };
